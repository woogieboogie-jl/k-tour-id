...
for (const profile of profiles) {
  ...
  test(`place rows preserve reading, visit and reservation returns ${profile.locale} ${profile.width}`, async ({ page, baseURL }, info) => {
    ...
    expect(await detail.getByTestId("experience-open").evaluate(node => Boolean(node.compareDocumentPosition(document.querySelector('[data-testid="canonical-meal-benefit-open"]')!) & Node.DOCUMENT_POSITION_FOLLOWING))).toBe(true)
    ...
    await expect(detail.getByTestId("canonical-place-table")).toContainText({ en: "Dining plans", ko: "식사 계획", ja: "食事プラン" }[profile.locale])
