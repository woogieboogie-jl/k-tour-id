...
**9/21 최종본 통합 — 먼저 읽기:** Harvey 최종 `f4526af3`를 기존 UX 위에 `integration/harvey-final-20260921`로 병합했습니다(로컬, 미푸시·미배포). [최종 통합·검수·남은 작업](./HARVEY_FINAL_INTEGRATION_2026-09-21.md)이 현재 구현 상태의 기준입니다. 아래는 가이드 저장 v2의 기획/개발 요구사항이며, 현재 연결된 Harvey 혜택 사용 v1과 동일 구현이 아닙니다. 실제 OpenDID는 미구현 경계를 유지하고, CX/체인 실환경 완료는 별도 확인해야 합니다. Harvey의 추가 개발을 기다리는 상태가 아니라 **우리 통합본의 검수·실환경 연결 단계**입니다.

**9/17 로컬 통합 업데이트:** Harvey의 `b151c745`와 후속 `84254d24`를 기존 모바일 후속본과 `integration/harvey-sync-20260917`에 통합했습니다. 미푸시·미배포이며 실연동 완료가 아닙니다. 현재 Harvey의 혜택 사용 v1과 아래 가이드 저장 v2의 계약은 아직 다릅니다. 실행 방법·격리 검수·남은 Sui/OpenDID 작업은 [로컬 통합 기록](./HARVEY_SYNC_LOCAL_2026-09-17.md)를 우선 확인하세요.
...
**별도 로컬 후속본:** 모바일 가독성·정확한 복귀·Table 일정·Pass 방문 기록·자격 공유 안내를 정리했습니다. 해커톤 필수 연동 범위는 그대로입니다. 9/17 통합 브랜치에는 `d55a31f0`로 보존했지만 미푸시·미배포이며 기존 원격 Harvey 브랜치에는 없습니다. [로컬 변경 계약과 검수 상태](./KTOUR_MOBILE_REFINEMENT_LOCAL_2026-09-16.md)를 확인하세요.
