...
> 후속 최종 상태: 두 KV 변수를 검증 브랜치 전용으로 수정했고, 로컬 및 실제 서울 Preview 각각 Redis primitive 11개 검사/테스트 키 정리를 통과했다. 임시 진단 경로·토큰·배포도 정리했다. [Redis 검증 기록](./REDIS_PREVIEW_VERIFICATION_2026-09-25.md)을 따른다. 아래 변수 미등록/생성 대기 문구는 각 시점의 이력이다. 앱 원장 및 CX 본인인증 완료를 의미하지 않는다.
